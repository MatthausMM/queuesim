...
public class vFinal {
    ...
    static void agendarProximoAtendimento(int indiceFilaOrigem) {
        ...
        if (logs) {
            ...
            System.out.println("Agendando atendimento para t=" + tempoAtend);
        }

        if (destino == -1) {
            // Saída do sistema
            if (logs) {
                System.out.println("evento adicionado: SAIDA, tempo=" + tempoAtend + ", filaOrigem=" + indiceFilaOrigem + ", -1");
            }
            esc.add(new Evento(Evento.EventType.TIPO_SAIDA, tempoAtend, indiceFilaOrigem, -1));
        } else {
            // Passagem para outra fila
            if (logs) {
                System.out.println("evento adicionado: PASSAGEM, tempo=" + tempoAtend + ", filaOrigem=" + indiceFilaOrigem + ", destino=" + destino);
            }
    ...
    static void CHEGADA(Evento evento) {
            ...
            if (origem.Status() <= origem.Server()) {
                if (origem.getProbs().length == 1 && origem.getProbs()[0] == 1.0 && origem.getDestinos()[0] == -1) {
                    ...
                    if (logs) {
                        ...
                        System.out.println("evento adicionado: SAIDA, tempo=" + saida + ", filaOrigem=" + evento.filaDestino + ", -1");
                    }
                    esc.add(new Evento(Evento.EventType.TIPO_SAIDA, saida, evento.filaDestino, -1));
                } else if (origem.getProbs().length == 1 && origem.getProbs()[0] == 1.0
                        && origem.getDestinos()[0] != -1) {
                    try {
                        saida = TempoGlobal + tempoAtendimento(evento.filaDestino);
                        if (logs) {
                            System.out.println("Agendando passagem para " + saida);
                            System.out.println("tempoAtendimento: " + (saida - TempoGlobal));
                            System.out.println("evento adicionado: PASSAGEM, tempo=" + saida + ", filaOrigem=" + evento.filaDestino + ", destino=" + origem.getDestinos()[0]);
                        }
                        esc.add(new Evento(Evento.EventType.TIPO_PASSAGEM, saida, evento.filaDestino,
                                origem.getDestinos()[0]));
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                } else {
        ...
        if (logs) {
            ...
            System.out.println("evento adicionado: CHEGADA, tempo=" + chegada + ", -1, filaDestino=" + evento.filaDestino);
        }
        ...
        if (origem.Status() >= origem.Server()) {
            if (origem.getProbs().length == 1 && origem.getProbs()[0] == 1.0 && origem.getDestinos()[0] == -1) {
                ...
                if (logs) {
                    System.out.println("Agendando saída para " + saida);
                    System.out.println("tempoAtendimento: " + (saida - TempoGlobal));
                    System.out.println("evento adicionado: SAIDA, tempo=" + saida + ", filaOrigem=" + evento.filaOrigem + ", -1");
                }
                esc.add(new Evento(Evento.EventType.TIPO_SAIDA, saida, evento.filaOrigem, -1));
            } else if (origem.getProbs().length == 1 && origem.getProbs()[0] == 1.0 && origem.getDestinos()[0] != -1) {
                try {
                    saida = TempoGlobal + tempoAtendimento(evento.filaDestino);
                    if (logs) {
                        System.out.println("Agendando passagem para " + saida);
    ...
    static void PASSAGEM(Evento evento) {
        ...
        if (origem.Status() >= origem.Server()) {
            /*
             * if (destino.getProbs().length == 1 && destino.getProbs()[0] == 1.0 &&
             * destino.getDestinos()[0] == -1) {
             * saida = TempoGlobal + tempoAtendimento(evento.filaDestino);
             * if (logs) {
             * System.out.println("Agendando saída para " + saida);
             * }
             * esc.add(new Evento(Evento.EventType.TIPO_SAIDA, saida, evento.filaDestino,
             * -1));
             * } else
             */ if (origem.getProbs().length == 1 && origem.getProbs()[0] == 1.0 && origem.getDestinos()[0] != -1) {
                try {
                    ...
                    if (logs) {
                        System.out.println("Agendando passagem para " + saida);
                        System.out.println("tempoAtendimento: " + (saida - TempoGlobal));
                        System.out.println("evento adicionado: PASSAGEM, tempo=" + saida + ", filaOrigem=" + evento.filaOrigem + ", destino=" + origem.getDestinos()[0]);
                    }
                    esc.add(new Evento(Evento.EventType.TIPO_PASSAGEM, saida, evento.filaOrigem,
                            origem.getDestinos()[0]));
                } catch (Exception e) {
                    e.printStackTrace();
                }
        ...
        if (destino.Status() < destino.Capacity()) {
            ...
            if (destino.Status() <= destino.Server()) {
                if (logs) {
                    System.out.println("destino.Status() = " + destino.Status() + " <= destino.Server() = " + destino.Server());
                }
    ...
    public static void main(String[] args) {
        ...
        // System.out.println("Configurando fila 1 (G/G/1) chegadas entre 2..4 e
        // atendimento entre 1..2");
        // fila fila1 = new fila(1, 1000000, 2.0, 4.0, 1.0, 2.0, new double[] { 0.2, 0.8
        // }, new int[] { 2, 1 });
        // System.out.println("Configurando fila 2 (G/G/2/5) atendimento entre 4..6");
        // fila fila2 = new fila(2, 5, 0.0, 0.0, 4.0, 6.0, new double[] { 0.3, 0.5, 0.2
        // }, new int[] { 0, 1, -1 });
        // // fila fila2 = new fila(2, 5, 0.0, 0.0, 4.0, 6.0, new double[] { 0.5, 0.3,
        // 0.2
        // // }, new int[] { 1, 0, -1 });
        // System.out.println("Configurando fila 3 (G/G/2/10) atendimento entre 5..15");
        // fila fila3 = new fila(2, 10, 0.0, 0.0, 5.0, 15.0, new double[] { 0.7, 0.3 },
        // new int[] { 2, -1 });
        ...
        // filas.add(fila3);
        Scanner in = new Scanner(System.in);
        System.out.println("Digite o nome do arquivo de modelo (ex: model.yml):");
        String caminho = "Tandem/allProb/" + in.nextLine();

        try {
            // Carregar configuração do arquivo YAML
            System.out.println("Carregando modelo de: " + caminho);

            lerModel modelo = new lerModel(caminho);
            ...
            if (chegadasExternas != null) {
                for (Map.Entry<String, Double> entry : chegadasExternas.entrySet()) {
                    ...
                    if (indiceFilaChegada >= 0) {
                        ...
                        System.out.println(
                                "Primeira chegada em " + nomeFilaChegada + " agendada para t=" + tempoChegadaInicial);
                    }
                }
            }
            ...
            for (int filaIndex = 0; filaIndex < filas.size(); filaIndex++) {
                ...
                if (f.Capacity() == 1000000)
                    System.out.println("Fila " + (filaIndex + 1) + ": (G/G/" + f.Server() + ")");
                else
